...
RSpec.describe Holt::Grid::Clearing do
  describe '.initialize' do
    it 'sets values for a clearing' do
      ...
      expect(clearing.adjacent_clearings).to eq([])
    end
  end

  describe '.add_path' do
    it 'creates a link between two adjacent clearings' do
      clearing_one = Holt::Grid::Clearing.new(priority: 1, suit: :bunny, slots: 2)
      clearing_two = Holt::Grid::Clearing.new(priority: 2, suit: :fox, slots: 2)

      clearing_one.add_path(clearing_two)
      expect(clearing_one.adjacent_clearings).to eq([clearing_two])
      expect(clearing_two.adjacent_clearings).to eq([clearing_one])
    end
  end
end
